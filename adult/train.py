...
def get_data(args):
    if args.no_finetuning:
        data_path = f"../data/adult/vectors_extracted_from_trained_models/{args.model}/pretrained/vectors_{args.model}_128.pt"
        ...
        data_path = f"../data/adult/tokens_{args.model}_128.pt"
...
def __main__():
    ...
    init_wandb(args, "Adult", "Adult final test")
    set_seed(args.seed)
    data_train, data_valid = get_data(args)

    checkpoint_folder = "../checkpoints/adult"
    ...
    for g in range(len(range_grads)):
        ...
        model, checkpoint_folder = get_test_model(args.no_finetuning, data_train.n_labels, checkpoint_folder, args.model,
                                             ...
                                             args.no_group_labels, args.control, grad)

        trainer = get_test_trainer(args, model, ['M', 'F'], metric='acc')
    ...
    trainer = get_trainer(args, model, ['M', 'F'], metric='acc', w=w)
