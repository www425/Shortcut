...
def init_wandb(args):
    name_of_project = "adult_age DFL Testing"
...
def __main__():
    ...
    if args.no_finetuning:
        ...
        data = FinetuningData(f"../data/adult_age/tokens_{args.model}_128.pt",
                              args.seed, args.split, args.testing_balanced, args.few_shot, groups=['<25', '>60', '25-60'])

    model, checkpoint_folder = get_model(args.no_finetuning, data.n_labels, "../checkpoints/adult_age", args.model,
    ...
    if args.no_finetuning:
        trainer = NoFinetuningClassificationTrainer(model, loss_fn, None, batch_size, ['<25', '>60', '25-60'], device=device)
    else:
        trainer = FinetuningClassificationTrainer(model, loss_fn, None, batch_size, ['<25', '>60', '25-60'], device=device)
