...
class ClassificationTrainer(Trainer, ABC):
    ...
    def evaluate(self, data: Data, split_type: str):
        ...
        print(f"{total_correct} / {total_examples}   {accuracy}")
...
# class FinetuningtestClassificationTrainer(ClassificationTrainer):
#
#     def forward_fn(self, batch):
...
class DFLtestClassificationTrainer(ClassificationTrainer):

    def __init__(self, model, loss_fn, optimizer, bsz, groups, gamma=2, alpha=1,
                 temperature=1,
    ...
    def forward_fn(self, batch, is_validation=False):
        ...
        logits, logits_b, features = self.model.forward(X, att_mask, return_features=True)
        features_np = features.detach().cpu().numpy()
        z_np = z.cpu().numpy()
        biased_loss = 0
        # print(f"z:{z}")
        # print(f"logits_b:{logits_b}")
        ...
        loss = self.loss_fn(logits, y)
        loss_all = (loss+biased_loss)/2


        n_correct = torch.sum(y == logits.argmax(dim=1)).item()

        return {'loss': loss_all, 'n_correct': n_correct, 'logits': logits}
...
class DFLClassificationTrainer(ClassificationTrainer):
    ...
    def __init__(self, model, loss_fn, optimizer, bsz, groups, biased_model, biased_optimizer, gamma=2, alpha=1,
                 temperature=1,
                 scheduler=None, device='cpu', no_group_labels=False, control=False, w=1):
        super().__init__(model, loss_fn, optimizer, bsz, groups, scheduler, device)
        self.dfl_loss_fn = FocalLoss(gamma=gamma, alpha=alpha, temperature=temperature)
    ...
    def forward_fn(self, batch, is_validation=False):
        ...
        loss = self.dfl_loss_fn(logits, y, logits_b, z, self.w)
        n_correct = torch.sum(y == logits.argmax(dim=1)).item()
