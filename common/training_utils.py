...
from models import deBERTa_classifier, Bert_classifier, Bert_classifier_test, deBERTa_classifier_test
from trainer import FinetuningClassificationTrainer, DFLClassificationTrainer, NoFinetuningClassificationTrainer, \
    DFLNoFinetuningClassificationTrainer,  DFLtestClassificationTrainer

def get_test_trainer(args, model, groups, metric):
    ...
    if args.no_finetuning:
        ...
        if args.dfl_gamma > 0:
            trainer = DFLtestClassificationTrainer(model, loss_fn, optimizer, args.batch_size, groups,
                                               gamma=args.dfl_gamma, temperature=args.temp, device=device,
...
def get_trainer(args, model, groups, metric, w):
    loss_fn = torch.nn.CrossEntropyLoss()
    ...
    if args.no_finetuning:
        if args.dfl_gamma > 0:
            biased_model = nn.Linear(model.in_features, len(groups)).to(device)
            biased_optimizer = Adam(biased_model.parameters(), lr=args.lr)
            trainer = DFLNoFinetuningClassificationTrainer(model, loss_fn, optimizer, args.batch_size, groups, biased_model,
                                               biased_optimizer, gamma=args.dfl_gamma, temperature=args.temp, device=device)
        else:
        ...
        if args.dfl_gamma > 0:
            biased_model = nn.Linear(model.classifier.in_features, len(groups)).to(device)
            ...
            trainer = DFLClassificationTrainer(model, loss_fn, optimizer, args.batch_size, groups, biased_model,
                                               biased_optimizer, gamma=args.dfl_gamma, temperature=args.temp, device=device,
                                               no_group_labels=args.no_group_labels, control=args.control, w=w)
        else:
            trainer = FinetuningClassificationTrainer(model, loss_fn, optimizer, args.batch_size, device=device,
                                                      groups=groups, metric=metric)
...
def get_test_model(no_finetuning, n_labels, checkpoint_folder_prefix, model_name, seed, data_type, balanced, dfl_gamma, temp,
              no_group_labels, control, hp_lambda, groups, fair_batch=None, fairness_type=None):
    ...
    if no_finetuning:
        ...
        if model_name == 'deberta-base':
            model = deBERTa_classifier_test(n_labels, hp_lambda, len(groups))
        if model_name == 'bert-base-uncased':
            model = Bert_classifier_test(n_labels, hp_lambda, len(groups))

        if dfl_gamma > 0:
            checkpoint_folder = f"{checkpoint_folder_prefix}/{model_name}/{folder}/{data_type}/{balanced}/gamma_{dfl_gamma}/temp_{temp}/seed_{seed}/{hp_lambda}"
        else:
            folder = 'DFL'
            checkpoint_folder = f"{checkpoint_folder_prefix}/{model_name}/{folder}/{data_type}/{balanced}/gamma_{dfl_gamma}/seed_{seed}"

    print(f"test_model will be saved to {checkpoint_folder}")
