...
def balance_dataset(X, y, z, masks=None, other=None, oversampling=False, groups=('M', 'F')):
    ...
    for label in np.unique(y):
        ...
        #     print(f"size={size} n_female={n_female} n_male={n_male}")
        #     sampled_female_idx = np.random.choice(female_idx, size=size, replace=False)
        #     sampled_male_idx = np.random.choice(male_idx, size=size, replace=False)
        #
        # indexes2 += sampled_male_idx.tolist()
        # indexes2 += sampled_female_idx.tolist()
        group_idx = []
        group_size = []
        for group in groups:
            idx_bool = np.logical_and(y == label, z == group)
            idx_n = np.sum(idx_bool)
            idx = np.arange(len(y))[idx_bool]
            group_size.append(idx_n)
            group_idx.append(idx.tolist())

        if oversampling:
            size = max(group_size)
            for i in range(len(groups)):
                if group_size[i] == size:
                    sampled_idx = np.random.choice(np.array(group_idx[i]), size=size, replace=False)
                else:
                    sampled_idx = np.random.choice(np.array(group_idx[i]), size=size, replace=True)
                indexes += sampled_idx.tolist()
...
def Adult_extract_tokens_data(tokenizer, model_name):
    ...
    for index, r in X.iterrows():
        sent = (
            ...
            f"relationship: {r['relationship']}, sex: {r['sex']}, race: {r['race']}, capital-gain: {r['capital-gain']}, "
            f"capital-loss: {r['capital-loss']}, hours-per-week: {r['hours-per-week']}, "
            f"native-country: {r['native-country']}"
        )

        inputs.append(sent)
        if r["income"].startswith("<=50K"):
            labels.append("<=50K")
        else:
            labels.append(">50K")
        genders.append(r["sex"][0])
    ...
    genders = np.array(genders)
    torch.save({"X": input_ids, "masks": attention_masks, "y": labels, "z": genders},
               f"../data/adult/tokens_{model_name}_128.pt")

def Adult_extract_tokens_data_1(tokenizer, model_name):
    adult = fetch_ucirepo(id=2)
    ...
    labels = np.array(labels)
    race = np.array(race)
    torch.save({"X": input_ids, "masks": attention_masks, "y": labels, "z": race},
               f"../data/adult_race/tokens_{model_name}_128.pt")

def Adult_extract_tokens_data_2(tokenizer, model_name):
    adult = fetch_ucirepo(id=2)
    ...
    labels = np.array(labels)
    age = np.array(age)
    torch.save({"X": input_ids, "masks": attention_masks, "y": labels, "z": age},
               f"../data/adult_age/tokens_{model_name}_128.pt")