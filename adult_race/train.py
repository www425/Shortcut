...
def get_data(args):
    if args.no_finetuning:
        ...
        data_path = f"../data/adult_race/tokens_{args.model}_128.pt"
...
def __main__():
    ...
    init_wandb(args, "Adult", "Adult_race train")
    set_seed(args.seed)
    data_train, data_valid = get_data(args)

    checkpoint_folder = "../checkpoints/adult_race_1"
    ...
    for g in range(len(range_grads)):
        ...
        model, checkpoint_folder = get_test_model(args.no_finetuning, data_train.n_labels, checkpoint_folder, args.model,
                                             ...
                                             args.no_group_labels, args.control, grad, groups=['W', 'O'])

        trainer = get_test_trainer(args, model, ['W', 'O'], metric='acc')
    ...
    trainer = get_trainer(args, model, ['W', 'O'], metric='acc', w=w)
