...
def init_wandb(args):
    name_of_project = "adult_race DFL Testing"
...
def __main__():
    ...
    if args.no_finetuning:
        data_path = f"../data/adult_race/vectors_extracted_from_trained_models/{args.model}/pretrained/vectors_{args.model}_128.pt"
        data = PretrainedVectorsData(data_path, args.seed, split=args.split, balanced=args.testing_balanced, few_shot=args.few_shot)
    else:
        data = FinetuningData(f"../data/adult_race/tokens_{args.model}_128.pt",
                              args.seed, args.split, args.testing_balanced, args.few_shot, groups=('W', 'O'))

    model, checkpoint_folder = get_model(args.no_finetuning, data.n_labels, "../checkpoints/adult_race_1", args.model,
    ...
    if args.no_finetuning:
        trainer = NoFinetuningClassificationTrainer(model, loss_fn, None, batch_size, ['W', 'O'], device=device)
    else:
        trainer = FinetuningClassificationTrainer(model, loss_fn, None, batch_size, ['W', 'O'], device=device)
